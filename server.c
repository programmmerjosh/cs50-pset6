...
char* indexes(const char* path)
{
    ...
    // the path directory will be stored in opened_dir
    DIR* opened_dir;
    
    struct dirent* dir;
    
    // open the path (directory)
    opened_dir = opendir(new_path);
    
    if (opened_dir)
    {
        while ((dir = readdir(opened_dir)) != NULL)
        {
            // store each filename into variable filename
            char* filename = dir->d_name;
            
            // if .php
            if ((strcasecmp(filename, "index.php") == 0))
            {
                char* output = malloc(path_len + 10);
                strcpy(output, new_path);
                strcat(output, "index.php\0");
                closedir(opened_dir);
                return output;
            }
            
            // if .html
            if ((strcasecmp(filename, "index.html") == 0))
            {
                char* output = malloc(path_len + 11);
                strcpy(output, new_path);
                strcat(output, "index.html\0");
                closedir(opened_dir);
                return output;
            }
        }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // if nothing in file, return false
    if (file == NULL)
    {
        return false;
    }
    
    // declare a buffer and size
    BYTE *buffer;
    size_t size;
    ...
    // read char by char and store in buffer, relloc'ing size as required
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        buffer = realloc(buffer, sizeof(BYTE) * (size + 1));
        buffer[size] = c;
        size++;
    }
...
const char* lookup(const char* path)
{
    ...
    // if .css
    char* found = strcasestr(path, ".css");
    if (found != NULL)
    {
        return "text/css\0";
    }
    ...
    // if .jpeg
    found = (strcasestr(path, ".jpg"));
    if (found != NULL)
    {
        return "image/jpeg\0";
    }
    found = (strcasestr(path, ".jpeg"));
    if (found != NULL)
    {
        return "image/jpeg\0";
    }
    ...
    // if .ico
    found = (strcasestr(path, ".ico"));
    if (found != NULL)
    {
        return "image/x-icon\0";
    }
    
    // if .js
    found = (strcasestr(path, ".js"));
    if (found != NULL)
    {
        return "text/javascript\0";
    }
    
    // if .php
    found = (strcasestr(path, ".php"));
    if (found != NULL)
    {
        return "text/x-php\0";
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* temp = malloc(LimitRequestLine + 1);
    ...
    while (j < line_len)
    {
        // only one space before/after get, req_target & version
        if ((line[j] == ' ') && (line[j + 1] == ' '))
        {
            error(400);
            return false;
    ...
    char* temp1 = malloc(LimitRequestLine + 1);
    char* req_target = malloc(LimitRequestLine + 1);
    strcpy(temp1, line);
    ...
    char* temp3 = malloc(LimitRequestLine + 1);
    char* the_query = malloc(LimitRequestLine + 1);
    strcpy(temp3, line);
